...
select 속성명들 from 테이블1
    ...
    [order by절]
    [limit절]
- 속성명1과 속성명2가 이름이 같으면 반드시 테이블명을 적어야함. 
...
select product.*, po_title, po_amount from product -- 뒤에 가져와서 붙일 수도 있음
	join product_option 
	on po_pr_num = pr_num;
    
    
-- abc 회원이 구매한 제품 옵션 번호를 조회. 단, 중복된 제품 옵션이 있으면 제거
-- 알고 있는 정보를 가진 테이블에서 시작하는게 편함 -> order를 기준으로
select distinct or_me_id as 구매자, ol_po_num as 제품옵션번호 from `order`
	join order_list
    on or_num = ol_or_num
    where or_me_id='abc';
...
select or_me_id as 아이디, ab_address as 배송지 from `order` 
	join address_book on ab_num = or_ab_num;
...
-- 분류별 등록된 제품명을 조회
	-- 원래 등록되지 않은 것(null값)이 있으면 아예 조회X -> right join을 쓰면 전부 뜸
select distinct mc_name as 분류, sc_name as 소분류, ifnull(pr_title,'등록된 제품없음')
	from product
