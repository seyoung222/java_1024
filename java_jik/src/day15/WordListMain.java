...
//			System.out.println(wordList);
		...
		case 5:
			...
			break;
		default:
			throw new Exception("예외 발생 : 잘못된 메뉴를 선택했습니다.");
	...
	private static void deleteWord(ArrayList<Word>wordList) throws Exception {
		...
		wordList.remove((int)indexList.get(index));//번지삭제를 위해 언박싱 주의
	...
	private static void updateWord(ArrayList<Word>wordList) throws Exception {
		...
		//단어장에서 입력 단어와 일치하는 단어 객체들을 가져옴(??)
			//실제 단어 번지: 1, 2, 4
			//새 리스트를 만들어서 0번지에:1, 1번지에:2, 2번지에:4
		ArrayList<Integer> indexList = findWordList(wordList, word);
//		System.out.println(indexList);
		...
		//get으로 가져오면 주소를 공유, tmp를 수정하면 원래것도 같이 바뀜
		tmp.setMean(mean);
		System.out.println("수정이 완료됐습니다.");
//		wordList.set(indexList.get(index), new Word(word, mean));//이렇게까지 안해도됨
	}
	private static ArrayList<Integer> findWordList(ArrayList<Word>wordList,String word){
		ArrayList<Integer> list = new ArrayList<Integer>();
		for(int i=0; i<wordList.size(); i++) {
			if(wordList.get(i).equals(word)) { 
				//문자열일때도 비교할 수 있게하려고 Word 클래스의 equals 메소드를 수정함
				list.add(i);
			}
		}
		return list;
