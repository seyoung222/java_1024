...
public class MethodArrayEx02 {
	...
	public static void main(String[] args) {
		/* 1~9 사이의 랜덤한 수를 배열에 3개 저장하는 코드를 작성하세요. 메소드를 이용할 것
		 * */
		int min=1, max=9;
		int arr1[] = new int[3];
	...
	/* 기능 : 최솟값과 최대값 사이의 랜덤한 수를 주어진 배열에 저장하는 메소드
	 * 매개변수 : 최솟값, 최대값, 배열 => int arr[], int min, int max
	 * 리턴타입 : 없음 => void
	 * 메소드명 : createRandomArray1
	 * */
	public static void createRandomArray1(int arr[],int min,int max) {
		for(int i=0; i<arr.length; i++) {
			arr[i] = random(min,max); //아래에서 만든 메소드를 활용
		}
	}
	/* 기능 : 최솟값과 최대값 사이의 랜덤한 수를 size개 만들어 배열에 저장한 후,
	 * 		 저장된 배열을 반환하는 메소드
	 * 매개변수 : 최솟값, 최댓값, 만들개수 => int min, int max, int size
...
//	}
//	
//	public static int[] random2(int size, int min, int max) {
//		int arr[] = new int[size];
//		for(int i=0; i<arr.length; i++) {
//			arr[i]=(int)(Math.random()*(max-min+1)+min);
//		}
