...
public class AccountServiceImp implements AccountService {
	...
	public Item inputItem(Scanner scan) throws ParseException {
		...
		System.out.print("목적(식비/교통비): ");
		String purpose=scan.next();
		scan.nextLine();
		System.out.print("내용: ");
		String content=scan.nextLine();
		System.out.print("금액(1000): ");
		int money =scan.nextInt();
		System.out.print("일시(2022-11-14):");
	...
	public String inputDate(Scanner scan) throws ParseException {
		//두줄 밖에 안되는 코드지만 예외처리를 위해서 메소드로 만듦
		System.out.print("일시 입력: ");
		String date = scan.next();
		SimpleDateFormat f = new SimpleDateFormat("yyyy-MM-dd");
		f.parse(date); //잘못입력하면 예외를 발생시키기 위해서 넣음
		return date;
	}
	@Override
	public void printItem(ArrayList<Item> list, int... dates) {
		...
		date = date.substring(1); //년도 앞의 -를 제거하는 역할
		
		int size = date.length();
		for(Item item : list) {
			if(item.getDateStr().substring(0,size).equals(date)) {
				//앞에서 4글자를 잘라서 date랑 같은지 비교, 같으면 출력함
				System.out.println(item);						
			}
		}		
	}
