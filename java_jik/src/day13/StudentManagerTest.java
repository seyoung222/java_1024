...
public class StudentManagerTest {
	...
	public static void main(String[] args) {
		...
		//메뉴 선택
			...
			//입력한 학생 정보가 없으면 추가
			//학생 정보들 중에서 하나씩 비교하여 학년,반,번호 같은 학생 있는지 확인해서
			//없으면 마지막 학생 뒤에 추가
				int index=-1;
				Student tmp = new Student(grade, classNum, num, name);
				for(int i=0; i<count; i++) {
					if(stds[i].equals(tmp)){
						index=i;//일치하는 학생의 위치(번지)를 알려주게 함
					}
				}
			...
			//선택한 학생의 성적 정보들 중에서 과목, 학기가 같은 성적 정보가 없으면 추가
				boolean res = stds[index].addScore(new Score(title, term, midScore, finScore, performance));
				if(res) {
